...
포트폴리오 + 블로그 사이트. 영상, 디자인, 코드 작업물을 정리하고 생각을 기록합니다.

**[davemins.com](https://davemins.com)** · ko / en / es
...
| | 기술 |
|---|---|
| 백엔드 | ASP.NET Core 8 (REST API) |
| 프론트엔드 | React + Vite + TypeScript |
| 콘텐츠 | Markdown + Frontmatter (파일 기반) |
| 배포 | AWS S3 + CloudFront (프론트) / EC2 (백엔드) |
...
CloudFront 설정:
- `/*` → S3 버킷
- `/api/*` → EC2 백엔드 (옵션: API URL 직접 사용 시 불필요)
